...
/*   Updated: 2026/06/30 13:17:16 by msowinsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_handle_specifiers(t_format *spec, va_list ap)
{
	...
	if (spec->conversion == 'c')
		return (ft_print_character(spec, va_arg(ap, char)));
	if (spec->conversion == 's')
		return (ft_print_string(spec, va_arg(ap, char *)));
	if (spec->conversion == 'p')
		return (ft_print_pointer(spec, va_arg(ap, uintptr_t)));
	...
	if (spec->conversion == 'x')
		return (ft_print_hex(spec, va_arg(ap, unsigned int), 0));
	if (spec->conversion == 'X')
		return (ft_print_hex(spec, va_arg(ap, unsigned int), 1));
	return (0);
}
